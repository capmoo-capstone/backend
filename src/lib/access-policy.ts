...
const { ADMIN, REPRESENTATIVE, HEAD_OF_DEPARTMENT, HEAD_OF_UNIT, DOCUMENT_STAFF, FINANCE_STAFF, GENERAL_STAFF } = UserRole
...
const supplyRolesForCapability: Partial<Record<Capability, UserRole[]>> = {
  ...
  [Capability.PROJECT_ADD_ASSIGNEE]: [
    GENERAL_STAFF,
    HEAD_OF_UNIT,
    ADMIN,
  ],
  ...
  [Capability.PROJECT_APPROVE_CANCELLATION]: [
    HEAD_OF_DEPARTMENT,
    HEAD_OF_UNIT,
  ],
  ...
  [Capability.SUBMISSION_PROPOSE]: [DOCUMENT_STAFF],
  [Capability.SUBMISSION_SIGN]: [DOCUMENT_STAFF],
